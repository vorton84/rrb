...
def fig1_seed_band():
    ...
    ax.axvspan(DAPO_MEAN - 2*DAPO_SD, DAPO_MEAN + 2*DAPO_SD, color=BAND2, lw=0, zorder=0)
    ax.axvspan(DAPO_MEAN - DAPO_SD,   DAPO_MEAN + DAPO_SD,   color=BAND1, lw=0, zorder=0)
    ax.axvline(DAPO_MEAN, color=INK, lw=0.9, zorder=1)
    for yi, p in zip(y, pooled):
        out = abs(p - DAPO_MEAN) > 2*DAPO_SD
        ax.plot(p, yi, "o", ms=5.5, mfc=VERM if out else BLUE, mec="white", mew=0.8, zorder=3)
    ax.set_yticks(y); ax.set_yticklabels(names)
    ax.set_xlabel("pooled AIME 2024+2025, mean@128 at steps 320/350/380 (%)")
    ax.set_xlim(21.8, 27.0); ax.grid(axis="y", visible=False)
    handles = [Patch(color=BAND1, label="DAPO mean $\\pm$ 1 seed SD"),
               Patch(color=BAND2, label="$\\pm$ 2 seed SD"),
               Line2D([0],[0], marker="o", color="none", mfc=BLUE, mec="white", ms=5.5, label="inside 2 SD"),
               Line2D([0],[0], marker="o", color="none", mfc=VERM, mec="white", ms=5.5, label="outside 2 SD")]
    ax.legend(handles=handles, loc="upper center", bbox_to_anchor=(0.5, -0.2), ncol=4, handlelength=1.4, columnspacing=1.2)
...
def fig5_dose_response():
    ...
    handles = [Patch(color=BAND1, label="DAPO $\\pm$ 1 seed SD"),
