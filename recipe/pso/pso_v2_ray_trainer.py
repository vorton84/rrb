...
class RayPSOV2Trainer(RayPPOTrainer):
    ...
    def _apply_pso_weights(self, batch):
        ...
        for uid in uid2all_idxs:
            ...
            if n_valid > 0 and pivotality[union_mask].max() > 0:
                ...
                is_pivotal = (pivotality >= threshold) & union_mask

                pivotal_piv = pivotality * is_pivotal.float()
                piv_max = pivotal_piv.max().clamp(min=1e-8)
                piv_scale = pivotal_piv / piv_max
